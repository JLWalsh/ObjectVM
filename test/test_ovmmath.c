...
void test_int16_endian_swap()
{
  ...
  int32_t big_endian = ovmmath_int16_endian_swap(small_endian);
...
void test_int64_endian_swap()
{
  ...
  int64_t big_endian = ovmmath_int64_endian_swap(small_endian);
  // Dirty hack: Enabling 64 bit support in unity (for TEST_ASSERT_EQUAL_INT64)
  // causes segfaults when using any other assertion after!
...
void test_int32_endian_swap()
{
  ...
  int32_t big_endian = ovmmath_int32_endian_swap(small_endian);
