...
void test_ovmbytecode_read_should_advance_bytecode_ptr_by_read_size()
{
    size_t read_size = 2;
    OVMUINT initial_bytecode_ptr = 1;
    ovm.bytecode_ptr = initial_bytecode_ptr;

    ovmbytecode_read(&ovm, read_size);

    TEST_ASSERT_EQUAL(initial_bytecode_ptr + read_size, ovm.bytecode_ptr);
}

void test_ovmbytecode_read_should_return_pointer_to_current_bytecode()
{
    ovm.bytecode_ptr = 2;

    void *ptr = ovmbytecode_read(&ovm, 1);

    TEST_ASSERT_EQUAL_PTR(&bytecode[2], ptr);
}

void test_ovmbytecode_read_op_should_read_op()
{
...
void main(void)
{
    ...
    RUN_TEST(test_ovmbytecode_read_should_advance_bytecode_ptr_by_read_size);
    RUN_TEST(test_ovmbytecode_read_should_return_pointer_to_current_bytecode);
    RUN_TEST(test_ovmbytecode_read_op_should_read_op);
    RUN_TEST(test_ovmbytecode_read_uint_should_read_uint);
